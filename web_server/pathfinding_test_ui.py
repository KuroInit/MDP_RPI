...
class PathfindingTestUI:
    ...
    def create_widgets(self):
        ...
        self.commands_text = tk.Text(control_frame, width=30, height=10)
    ...
    def draw_actual_route(self, commands):
        ...
        for cmd in commands:
            if cmd.startswith("SF") or cmd.startswith("SB"):
                ...
                self.draw_point_world(x, y, tag="actual_route", color="orange", size=4)
            elif cmd == "FIN":
                break
    ...
    def draw_point_world(self, x, y, tag, color, size=3):
        ...
        self.canvas.create_oval(cx-size, cy-size, cx+size, cy+size, fill=color, tags=tag)
    # 辅助：绘制小车头部（三角形）
