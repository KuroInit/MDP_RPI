    ...
    def get_view_state(self, retrying) -> List[CellState]:
        ...
        if self.direction == Direction.NORTH:
            if retrying == False:
                ...
                # Or (x, y + 5)
                if isValid(self.x, self.y + 3 + EXPANDED_CELL * 2):
                    cells.append(CellState(
        ...
        # If obstacle is facing east, then robot's cell state must be facing west
            ...
            if retrying == False:
                ...
                # Or (x + 3,y + 1)
                # if isValid(self.x + 1 + EXPANDED_CELL * 2, self.y + 1):
                ...
                # Or (x + 4, y + 1)
                if isValid(self.x + 2 + EXPANDED_CELL * 2, self.y + 1):
                ...
                if isValid(self.x + 2 + EXPANDED_CELL * 2, self.y):
                    cells.append(CellState(self.x + 2 + EXPANDED_CELL * 2,
                                 self.y, Direction.WEST, self.obstacle_id, 0))
                # Or (x + 5, y)
                if isValid(self.x + 3 + EXPANDED_CELL * 2, self.y):
        ...
        # If obstacle is facing west, then robot's cell state must be facing east
            ...
            # It can be (x - 2,y)
            # if isValid(self.x - EXPANDED_CELL * 2, self.y):
...
class Grid:
    ...
    def isValid_coord(self, x: int, y: int) -> bool:
    ...
    def isValid_cell_state(self, state: CellState) -> bool:
        ...
        Returns:
            bool: True if valid, False otherwise
        """
        return self.isValid_coord(state.x, state.y)
